...
lib.upload = function(aws, groupName, streamName, logEvents, retentionInDays, options, cb) {
  ...
  var all_context = new Set();
  all_context = logEvents.map(l => l.context);
  all_context.forEach(x => {
    var logs = logEvents.filter(s => s.context === x);
    this.uploadEvents(aws, groupName+'/'+x, streamName, logs, retentionInDays, options, cb);
  });
}
