...
lib.upload = function(aws, groupName, streamName, logEvents, retentionInDays, options, cb) {
  ...
  var all_context = new Set(logEvents.map(l => l.message.split('|')[0]));
  all_context.forEach(context => {
    var logs = logEvents.filter(s => s.message.split('|')[0] === context);
    this.uploadEvents(aws, groupName, streamName+'-'+context, logs, retentionInDays, options, cb);
  });
}

lib.uploadEvents = function(aws, groupName, streamName, logEvents, retentionInDays, options, cb) {
  debug('upload', logEvents);
